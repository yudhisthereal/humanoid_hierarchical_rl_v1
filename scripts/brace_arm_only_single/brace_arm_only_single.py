...
# arms-only: exclude leg/thigh/shin actuators and joints
ACTUATOR_NAMES = ["pos_waist", "pos_arm", "pos_forearm"]
JOINT_NAMES = ["waist_joint", "arm_joint", "forearm_joint"]
...
class HumanoidEnv:
	def __init__(self, xml_path: Path | str = XML_PATH):
		...
		self.arm_geom_ids = _collect_geom_ids(["uarm_geom", "larm_geom", "uarm_left_geom", "larm_left_geom"])
		...
		# BRACE-focused observation for arm-only setup with joint velocities
		# [com_z, arm_angle, forearm_angle,
	...
	def _get_obs(self) -> np.ndarray:
		arms_contact = 1.0 if self._has_floor_contact(self.arm_geom_ids) else 0.0
	...
	def map_action_to_ctrl(self, action: np.ndarray | "torch.Tensor") -> np.ndarray:
		"""Map normalized action in [-1, 1] to actuator control units.
		Supports numpy arrays and torch tensors.
		"""
		if isinstance(action, np.ndarray):
			a = np.nan_to_num(action.astype(np.float32), nan=0.0, posinf=1.0, neginf=-1.0)
			ctrl = self.ctrl_min + 0.5 * (a + 1.0) * (self.ctrl_max - self.ctrl_min)
			return np.clip(ctrl, self.ctrl_min, self.ctrl_max)
		try:
			import torch
			if isinstance(action, torch.Tensor):
				a = torch.nan_to_num(action.float(), nan=0.0, posinf=1.0, neginf=-1.0)
				ctrl_min = torch.as_tensor(self.ctrl_min, dtype=a.dtype, device=a.device)
				ctrl_max = torch.as_tensor(self.ctrl_max, dtype=a.dtype, device=a.device)
				ctrl = ctrl_min + 0.5 * (a + 1.0) * (ctrl_max - ctrl_min)
				return torch.clamp(ctrl, ctrl_min, ctrl_max)
		except Exception:
			pass
		# fallback: try to coerce to numpy
		arr = np.asarray(action, dtype=np.float32)
		arr = np.nan_to_num(arr, nan=0.0, posinf=1.0, neginf=-1.0)
		ctrl = self.ctrl_min + 0.5 * (arr + 1.0) * (self.ctrl_max - self.ctrl_min)
		return np.clip(ctrl, self.ctrl_min, self.ctrl_max)
	...
	def reset(self) -> np.ndarray:
		...
		# Set deterministic initial arm poses
		try:
	...
	def _brace_reward(self, success: bool) -> tuple[float, dict[str, float]]:
		...
		if reward_ver != "v1":
			# original reward simplified (no knee timing)
			...
			reward = r_arm_first + 0.8 * r_arm_sync - c_impact
			if success:
				reward += 100.0
	...
	def step(self, action: np.ndarray) -> tuple[np.ndarray, float, bool, dict]:
		# accept normalized actions in [-1, 1] and map to actuator ctrl units
		# preserve torch.Tensor support via map_action_to_ctrl
		try:
			ctrls = self.map_action_to_ctrl(action)
		except Exception:
			# fallback: coerce to numpy then map
			arr = np.nan_to_num(np.asarray(action, dtype=np.float32), nan=0.0, posinf=1.0, neginf=-1.0)
			ctrls = self.ctrl_min + 0.5 * (arr + 1.0) * (self.ctrl_max - self.ctrl_min)
		# write controls to model
		for i, aid in enumerate(self.actuator_ids):
			self.data.ctrl[aid] = float(ctrls[i])
		...
		arms_contact = self._has_floor_contact(self.arm_geom_ids)
		arms_contact_l = arms_contact
		arms_contact_r = arms_contact
...
def main():
	parser = argparse.ArgumentParser(description="Train arms-only brace PPO (single env).")
	...
	plot = LivePlot("Brace Arms-Only PPO Training Progress")
	...
	try:
		for iteration in range(max_iters):
			...
			if (iteration + 1) % 10 == 0:
				checkpoint_path = run_dir / f"brace_arm_model_iter_{iteration + 1}.pth"
			...
			prefix = "\x1b[1;32m[BRACE-ARM PPO]\x1b[0m"
			...
			print(
				f"\r{prefix}\n"
				f"{kv('iter', iter_txt, 'eps', eps_txt)}\n"
				f"{kv('steps', steps_txt, 'done_iter', done_iter_txt)}\n"
				f"{kv('mean_rew', mean_rew_txt, 'max_mean_rew', max_rew_txt)}\n"
				f"{kv('win_rate', f'{latest_success_rate * 100.0:.2f}%', 'max_win_rate', f'{max_win_rate:.2%}')}\n"
				f"{kv('consec_success', f'{consec_success}', 'max_consec', f'{max_consec}')}\n"
				f"{kv('entropy_coef', f'{agent.entropy_coef:.6f}', '', '')}",
				end="",
				flush=True,
			)
