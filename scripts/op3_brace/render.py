"""Render script for Op3BraceEnv (full-body).

Usage:
    python render.py --checkpoint checkpoints/full_body/best_op3_brace_model.pth --output_dir demo_full_body
"""
...
def main():
    parser = argparse.ArgumentParser(description="Render Op3BraceEnv (full-body) with a trained checkpoint (three POVs).")
        ...
        while step < episode_max:
            if not impacted:
                ...
                if done:
                    ...
                    try:
                        all_ctrl = env.map_action_to_ctrl(zero_action)
                        env._apply_controls(all_ctrl)
                    except Exception as e:
