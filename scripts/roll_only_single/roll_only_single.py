...
BEST_MODEL_PATH = CHECKPOINT_DIR / "best_roll_model.pth"
...
class HumanoidEnv:
	def __init__(self, xml_path: Path | str = XML_PATH):
		...
		self.waist_qpos_idx = int(
			self.model.jnt_qposadr[mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_JOINT, "waist_joint")]
		)
		...
		self.push_force_choices = np.array([20.0, 35.0, 70.0], dtype=np.float32)
		self.push_steps = 5
	...
	def _refresh_state(self) -> None:
		...
		self.waist_angle = self._get_joint_pos(self.waist_qpos_idx)
	...
	def reset(self) -> np.ndarray:
		...
		sampled = np.random.randint(0, len(self.push_force_choices))
		self.push_force = float(self.push_force_choices[sampled])
	...
	def _roll_reward(self, action: np.ndarray, success: bool) -> tuple[float, dict[str, float]]:
		r_vel = float(np.tanh(self.vx / 3.0))
		v_rot = -self.omega
		r_rot = float(np.tanh(np.clip(v_rot, 0.0, None) / 5.0))
		r_tuck = float(np.tanh((abs(self.waist_angle) + abs(self.knees_angle)) / 4.0))
		c_impact = max(0.0, 0.15 - self.head_z) * 5.0
		c_ctrl = float(np.tanh(np.sum(np.clip(action, -1.0, 1.0) ** 2) / 13.0))

		reward = r_vel + r_rot + 0.5 * r_tuck - 0.5 * c_ctrl - c_impact
		if success:
			reward += 100.0
	...
	def step(self, action: np.ndarray) -> tuple[np.ndarray, float, bool, dict]:
		...
		for i, aid in enumerate(self.actuator_ids):
			...
			self.data.ctrl[aid] = np.clip(action[i], float(ctrl_range[0]), float(ctrl_range[1]))
		...
		roll_success = (self.rotation >= (1.5 * np.pi)) and (((max(0.0, 0.15 - self.head_z) * 100.0)) < 0.1)

		reward, reward_parts = self._roll_reward(action, roll_success)
		...
		self.last_success = bool(roll_success)
		self.done = bool(roll_success or stopped or timeout or invalid_state)
		self.step_count += 1
...
def main():
	parser = argparse.ArgumentParser(description="Train roll-only PPO (single env).")
	...
	plot = LivePlot("Roll Only PPO Training Progress")
	...
	try:
		for iteration in range(max_iters):
			...
			if mean_reward > max_mean_reward:
				...
				best_model_path = run_dir / "best_roll_model.pth"
				torch.save(agent.network.state_dict(), best_model_path)

			if (iteration + 1) % 10 == 0:
				checkpoint_path = run_dir / f"roll_model_iter_{iteration + 1}.pth"
			...
			prefix = "\x1b[1;32m[ROLL PPO]\x1b[0m"
