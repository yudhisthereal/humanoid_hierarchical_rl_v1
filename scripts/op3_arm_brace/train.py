...
from envs.op3_arm_brace import Op3ArmBraceEnv
...
def main():
    ...
    reward_tracker = RewardComponentTracker(max_history=50, weights=reward_weights)
    ...
    try:
        for iteration in range(max_iters):
            ...
            # Update agent after collecting trajectory
            ...
            if mean_reward > max_mean_reward:
                ...
                best_model_path = run_dir / "best_op3_arm_brace_model.pth"
                torch.save(agent.network.state_dict(), best_model_path)

            # Periodic checkpoints
            if (iteration + 1) % 10 == 0:
                checkpoint_path = run_dir / f"op3_arm_brace_model_iter_{iteration + 1}.pth"
                torch.save(agent.network.state_dict(), checkpoint_path)

            max_win_rate = max(max_win_rate, latest_success_rate)
            
            # Finalize iteration for reward tracker
            ...
            # Print status
            prefix = "\x1b[1;32m[OP3 ARM BRACE PPO]\x1b[0m"
        ...
        reward_tracker.save(reward_component_plot_path, title="OP3 Arm-Only Brace - Reward Components")
